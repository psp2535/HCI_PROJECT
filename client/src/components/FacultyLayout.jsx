...
import { LayoutDashboard, Users, LogOut, ChevronRight, BookOpen, Menu, X } from 'lucide-react';
...
export default function FacultyLayout({ children }) {
  ...
  const navItems = [
    ...
    { to: '/faculty/courses', icon: BookOpen, label: 'Course Registrations' },
  ];
    ...
    <div className="flex min-h-screen">
      ...
      >
        <div style={{ width: '240px', minWidth: '240px' }} className="flex flex-col h-full">
          ...
          <div className="mx-3 mt-3 p-3 rounded-xl" style={{ background: 'rgba(124,58,237,0.1)', border: '1px solid rgba(124,58,237,0.18)' }}>
            <div className="flex items-center gap-3">
              <div className="w-9 h-9 rounded-full flex items-center justify-center text-white font-bold text-sm shrink-0" style={{ background: 'linear-gradient(135deg, #7c3aed, #8b5cf6)' }}>
                {user?.name?.charAt(0)}
              </div>
              <div className="min-w-0">
                <p className="text-white font-semibold text-sm truncate">{user?.name}</p>
                <p className="text-purple-300 text-xs mt-0.5 truncate">{user?.employeeId}</p>
              </div>
            </div>
            <span className="inline-block mt-2 text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'rgba(124,58,237,0.25)', color: '#c4b5fd' }}>Faculty · {user?.department}</span>
          </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden min-w-0">
        <header className="flex items-center justify-between px-5 py-3 shrink-0" style={{ background: 'var(--header-bg)', borderBottom: '1px solid var(--color-border)', backdropFilter: 'blur(20px)' }}>
          <div className="flex items-center gap-3">
            ...
            <h1 className="font-semibold text-base" style={{ color: 'var(--color-text)' }}>Academic Approval Panel</h1>
          </div>
