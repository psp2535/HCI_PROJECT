...
import { GraduationCap, LayoutDashboard, CheckSquare, List, LogOut, ChevronRight, Shield } from 'lucide-react';
    ...
    <div className="flex min-h-screen">
      <aside className="w-64 flex flex-col" style={{ background: 'rgba(15,23,42,0.95)', borderRight: '1px solid rgba(255,255,255,0.06)' }}>
        <div className="p-5" style={{ borderBottom: '1px solid rgba(255,255,255,0.06)' }}>
          <div className="flex items-center gap-3">
            <div className="w-9 h-9 rounded-lg flex items-center justify-center" style={{ background: 'linear-gradient(135deg, #059669, #10b981)' }}>
        ...
        <div className="p-4 mx-3 mt-3 rounded-xl" style={{ background: 'rgba(5,150,105,0.1)', border: '1px solid rgba(5,150,105,0.2)' }}>
          <p className="text-white font-semibold text-sm">{user?.name}</p>
          <p className="text-emerald-300 text-xs mt-0.5">{user?.employeeId}</p>
          <span className="inline-block mt-1 text-[10px] px-2 py-0.5 rounded-full" style={{ background: 'rgba(5,150,105,0.25)', color: '#6ee7b7' }}>Verification Staff</span>
        </div>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        <header className="flex items-center justify-between px-6 py-4" style={{ background: 'rgba(15,23,42,0.8)', borderBottom: '1px solid rgba(255,255,255,0.06)', backdropFilter: 'blur(20px)' }}>
          <h1 className="text-white font-semibold text-lg">Payment Verification Dashboard</h1>
